...
class Conversation {
    constructor({ flows = {}, locales = {}, platform, dialog = {}, voice = {}, trackingConfig = {}, trackingDataCollector, }) {
        this._output = [];
    ...
    translate(path, params = []) {
        let msgSrc = lodash_1.get(this._locales[this._locale], path);
        if (!msgSrc) {
    ...
    next(intent) {
        ...
        let [flowName, intentName] = intent.split(Kiai_1.INTENT_DELIMITER);
        this.currentFlow = flowName;
        const handler = this._flows[flowName][intentName];
        if (typeof handler !== 'function') {
            throw new Error(`Target intent not found: "${flowName}${Kiai_1.INTENT_DELIMITER}${intentName}"`);
        }
    ...
    track(event, data) {
        this._tracker = this._tracker || new Tracking_1.default({ config: this._tracking, userId: this.userId });
        let userData;
        ...
        return new Promise(resolve => {
            const executeHandler = () => {
                if (!this._handlers.length)
                    ...
                    Promise.resolve(this._handlers.shift()(this)).then(() => executeHandler());
            };
    ...
    resolveIntent(intent) {
        ...
        return `${flowName}${Kiai_1.INTENT_DELIMITER}${intentName}`;
    }
}
