...
class Conversation {
    ...
    next(intent) {
        ...
        let [flowName, intentName] = intent.split(Kiai_1.default.INTENT_DELIMITER);
        this.currentFlow = flowName;
        const handler = this.flows[flowName][intentName];
        if (typeof handler !== 'function') {
            throw new Error(`Target intent not found: "${flowName}${Kiai_1.default.INTENT_DELIMITER}${intentName}"`);
        }
    ...
    resolveIntent(intent) {
        ...
        return `${flowName}${Kiai_1.default.INTENT_DELIMITER}${intentName}`;
    }
}
