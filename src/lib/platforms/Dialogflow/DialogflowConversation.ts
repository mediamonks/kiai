...
export default class DialogflowConversation extends Conversation {
	...
	public play(sound: string, fallback = ''): Conversation {
		return this.add(`<audio src="${this.getAssetUrl('sfx', sound)}">${fallback}</audio>`);
	}

	public speak(voice: string, text = ''): Conversation {
	...
	public login(callbackIntent: string, speech = ''): Conversation {
	...
	public enableDailyNotification(
		intent: string,
		callbackIntent = `:${this.currentIntent}`,
		payload: TMapping = {},
	): Conversation {
	...
	public showCard({
		...
		const imageUrl = image && this.getImageUrl(image);
	...
	public carousel({
		...
		items.forEach(item => {
			...
			const listItem = { ...item, image: new Image(item.image) };
